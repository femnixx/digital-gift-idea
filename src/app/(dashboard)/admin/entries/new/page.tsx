...
export default function NewEntryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isDemoMode = !(process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY && process.env.NEXT_PUBLIC_SUPABASE_URL !== 'your_supabase_project_url')
    if (isDemoMode) {
      try {
      ...
      try {
        const response = await fetch('/api/entries', {
          ...
          body: JSON.stringify({ title, type: selectedType, slug, content: {}, is_published: true }),
        })
        if (!response.ok) {
        ...
        <div>
          ...
          <form onSubmit={handleSubmit} className='space-y-6'>
            <div>
              ...
              <input type='text' value={title} onChange={handleTitleChange} placeholder='Give your entry a beautiful title...' className='input' required />
            </div>
            <div>
              <label className='label'>Slug (URL)</label>
              <input type='text' value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^\w-]+/g, '-'))} placeholder='my-love-letter' className='input font-mono text-sm' required />
            ...
            <div className='flex items-center gap-4 p-4 rounded-xl bg-rose-50 border border-rose-100'>
              <input type='checkbox' id='published' defaultChecked className='w-5 h-5 rounded border-rose-300 text-rose-600 focus:ring-rose-500' />
            ...
            <div className='flex gap-4 pt-4'>
              ...
              <button type='button' onClick={() => setSelectedType(null)} className='btn-secondary'>Back</button>
            </div>
          </form>
        </div>
      </div>
